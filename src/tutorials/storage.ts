...
function builder(level: number) {
  // When the size of your game gets past a certain size, you'll find that it is
  // hard to use the `()=>{}` syntax to effectively keep track of all of the
  // information that a level of your game needs.  The "level" storage feature
  // lets you store information so that it is "globally accessible" throughout
  // your code.  It gets reset each time JetLag calls your builder() code.
  ...
  // the game with unconverted coins, they're lost, but rubies persist forever.

  // We'll also print welcome messages for "first time playing" and "first time
  // playing today".
  ...
  // Only set up persistent storage if we don't have one already. Note that for
  // our "first time playing" rules, we need this to be a bit more complex than
  // session storage.

  // Persistent storage is trickier, because you have to explicitly save it back
  // to persistent after any change.
