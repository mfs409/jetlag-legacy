...
import { Goodie, Hero, Obstacle, Projectile } from "../jetlag/Components/Role";
 ...
 *
 * NB:  See `game/game.ts` for more guidance on these fields
 */
export class Config implements GameConfig {
  ...
  musicNames = [];
  soundNames = [];
  imageNames = ["pirate.json", "noise.png"];

  gameBuilder = tut_overhead_fight_farm_game;
}
 ...
 */
export function tut_overhead_fight_farm_game(level: number) {
  level = 2;
  if (level == 1) {
    ...
    // Log the day
    stage.storage.setSession("day", 1 + (stage.storage.getSession("day") ?? 0));
    Actor.Make({
        ...
        heroCollision: () => {
          stage.requestOverlay((overlay: Scene) => {
            Actor.Make({
              ...
              gestures: { tap: () => { stage.clearOverlay(); stage.switchTo(tut_overhead_fight_farm_game, 1); return true; } },
            });
            Actor.Make({
    ...
    let projectiles = new ActorPoolSystem();
    populateProjectilePool(projectiles, {
      size: 1, strength: 1, range: .5, immuneToCollisions: true, disappearOnCollide: true, gravityAffectsProjectiles: false,
      bodyMaker: () => new CircleBody({ radius: 0.125, cx: -100, cy: -100 }, stage.world, { density: 0.01 }),
      appearanceMaker: () => new FilledCircle({ radius: 0.125, fillColor: "#777777", z: 0 }),
    });
