...
function extract(r: RegExp, content: string) {
 ...
 */
function extractClassFromContent(content: string) { return extract(/@class="([\S\s]+)"/, content); }

/**
 * Pull the first word out of a content string so we can treat it as a CSS class
 */
function extractFirstFromContent(content: string) { return extract(/([\S]+)/, content); }
...
let extVega = {
    type: "lang",
    regex: /(```vega)([\s\S]+?)(```)/g,
    idx: 0, // NB: we could use _location: number, but a counter is more deterministic
    ...
    //      of `text`.
    replace: function (_whole: string, _left: string, content: string, _right: string, _location: number, _text: string) {
        ...
        embed(d, JSON.parse(cleanedContent)).then(_value => {
 ...
 */
function wrapMarkdownInSections(className: string) {
    // We start by extracting, in order, all children from the body of the doc
    let oldChildren: Element[] = []
    ...
    let currDiv = document.createElement("div"), i = 0;
    currDiv.id = "div-" + i++;
...
// To keep TS happy, declare that there's going to be a global var called
// `preventAutoFetch`.  It's undefined by default, but a tutorial's HTML file
// can set it.
declare var preventAutoFetch: any;
...
(window as any).replaceDocument = replaceDocument;
(window as any).fetchMarkdownAndReplaceDocument = fetchMarkdownAndReplaceDocument;