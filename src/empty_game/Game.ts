...
export function build_game(_level: number) {
  ...
  for (let x = 0; x <= 16; x += 1) {
    Actor.Make({
      rigidBody: RigidBodyComponent.Box({ cx: x, cy: 4.5, width: 0.01, height: 9 }, stage.world),
      appearance: FilledSprite.Box({ width: 0.01, height: 9, lineWidth: 1, lineColor: "#000000" })
    });

    if (x < 16)
      Actor.Make({
        rigidBody: RigidBodyComponent.Box({ cx: x + .5, cy: 4.5, width: 0.01, height: 9 }, stage.world),
  ...
  for (let y = 0; y <= 9; y += 1) {
    Actor.Make({
      rigidBody: RigidBodyComponent.Box({ cx: 8, cy: y, width: 16, height: 0.01 }, stage.world),
      appearance: FilledSprite.Box({ width: 16, height: 0.01, lineWidth: 1, lineColor: "#000000" })
    });

    if (y < 9)
      Actor.Make({
        rigidBody: RigidBodyComponent.Box({ cx: 8, cy: y + .5, width: 16, height: 0.01 }, stage.world),
